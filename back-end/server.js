...
app.post("/api/services", authenticateToken, async (req, res) => {
  if (req.user.role !== "Service Provider") {
    return res
      .status(403)
      .json({ message: "Only Service Providers can create services." });
  }
  const provider_id = req.user.id;
  const { service_name, description, category, price, location, image_url } =
    req.body;
  if (!service_name) {
...
app.get("/api/services", async (req, res) => {
  ...
  if (category) {
    whereConditions.push("s.category LIKE ?");
    params.push(`%${category}%`);
  }
  if (keyword) {
    whereConditions.push("(s.service_name LIKE ? OR s.description LIKE ?)");
...
app.get("/api/availability/:provider_id/:date", async (req, res) => {
  ...
  try {
    ...
    const bookedTimes = new Set(
      bookings.map((b) => format(new Date(b.booking_start_time), "HH:mm:ss"))
    );
...
app.put(
  ...
  async (req, res) => {
    if (req.user.role !== "Service Provider") {
      return res
        .status(403)
        .json({ message: "Only providers can update booking status." });
    }
...
app.post("/api/reviews", authenticateToken, async (req, res) => {
  if (req.user.role !== "Customer") {
    return res
      .status(403)
      .json({ message: "Only customers can leave reviews." });
  }
  ...
  if (!booking_id || !service_id || !provider_id || !rating) {
    return res
      .status(400)
      .json({ message: "Missing required fields for review." });
  }
  ...
  try {
    ...
    if (bookingRows.length === 0) {
      return res
        .status(403)
        .json({ message: "You can only review your own completed bookings." });
    }
