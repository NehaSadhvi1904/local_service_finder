...
const Sidebar = ({
  ...
  <nav className="sidebar">
    ...
    <button onClick={onSignOut} className="sidebar-link signout-btn-sidebar">
      <PowerIcon /> Sign Out
    </button>
  </nav>
);
...
const DashboardOverview = ({ services, bookings }) => {
  ...
  const StatCard = ({ icon, title, value, color }) => (
    <div className="stat-card" style={{ borderColor: color }}>
      <div
        className="stat-icon"
        style={{ background: `linear-gradient(45deg, ${color}99, ${color}FF)` }}
      >
        {icon}
      </div>
    ...
    <div className="table-wrapper">
      {services.length === 0 ? (
        <p className="empty-state">
          No services yet. Add one from the sidebar.
        </p>
      ) : (
                ...
                <td>
                  <div className="service-name-cell">
                    <img
                      src={
                        s.image_url ||
                        `https://placehold.co/100x100/18191E/A797F7?text=${s.service_name.charAt(
                          0
                        )}`
                      }
                      alt={s.service_name}
  ...
  return (
    <section className="content-panel">
      <h3 className="panel-header">
        <PlusIcon /> Add a New Service
      </h3>
      <p className="panel-subtitle">
        Services require admin approval before they are visible to customers.
      </p>
...
const ScheduleEditor = ({ schedule, setSchedule, axiosWithAuth }) => {
  ...
  return (
    <section className="content-panel">
      <h3 className="panel-header">
        <CalendarIcon /> My Weekly Schedule
      </h3>
      <p className="panel-subtitle">
        Set your available hours. Customers can only book within these times.
      </p>
...
const BookingList = ({ bookings, axiosWithAuth, onStatusChange }) => {
  const handleBookingStatusChange = async (bookingId, newStatus) => {
    try {
      await axiosWithAuth.put(`/bookings/${bookingId}/status`, {
        status: newStatus,
      });
  ...
  const sortedBookings = useMemo(() => {
    return [...bookings].sort(
      (a, b) => new Date(b.booking_start_time) - new Date(a.booking_start_time)
    );
  }, [bookings]);
                ...
                <div className="provider-card-details">
                  ...
                  {b.review_id && (
                    <div className="review-display-provider">
                      ...
                      {b.comment && (
                        <p className="review-comment-provider">"{b.comment}"</p>
                      )}
                    </div>
                  )}
                </div>
...
const ProviderDashboard = () => {
  ...
  const renderActiveTab = () => {
    if (loading)
      return (
        <div className="full-page-loader" style={{ background: "none" }}>
          <div className="loader"></div>
        </div>
      );
    switch (activeTab) {
      case "dashboard":
        return <DashboardOverview services={services} bookings={bookings} />;
      case "services":
        return (
          <ServiceList
            services={services}
      ...
      case "bookings":
        return (
          <BookingList
            bookings={bookings}
            axiosWithAuth={axiosWithAuth}
            onStatusChange={() => fetchProviderData(user.id)}
          />
        );
      default:
        return null;
            ...
            <form onSubmit={handleUpdateService}>
              <div className="modal-form-grid">
                ...
                <div className="form-group">
                  ...
                  >
                    {FALLBACK_AVAILABILITIES.map((opt) => (
